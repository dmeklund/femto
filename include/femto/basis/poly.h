...
// e.g., for x^2*y + 3.5*x*y^2 + 1.5*y^3:
// fto_basis_poly2d_init(poly, 3, /* x^2*y */ 1.0, 2, 1, /* 3.5*x*y^2 */ 3.5, 1, 2, /* 1.5*y^3 */ 1.5, 0, 3)
extern enum FtoError fto_basis_poly2d_init(struct FtoPoly2D *poly, int num_elements, ...);
extern double fto_basis_poly2d_eval(const struct FtoPoly2D *poly, double pt_x, double pt_y);
extern enum FtoError fto_basis_poly2d_diff(
