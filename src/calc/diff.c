...
extern enum FtoError fto_calc_grad_piecewise2d(const struct FtoPolyPiecewise2D *poly, struct FtoVectorFunc *grad_out)
{
    enum FtoError ret;
    grad_out->funcs = fto_malloc(2 * sizeof *grad_out->funcs);
    grad_out->order = 2;
    struct FtoPolyPiecewise2D *dx = fto_malloc(sizeof *dx);
    struct FtoPolyPiecewise2D *dy = fto_malloc(sizeof *dy);
    if ((ret = fto_poly_piecewise2d_diff(poly, 0, dx)) != FTO_OK) return ret;
    if ((ret = fto_poly_piecewise2d_diff(poly, 1, dy)) != FTO_OK) return ret;
    if ((ret = fto_function_fromPolyPiecewise2D(dx, &grad_out->funcs[0])) != FTO_OK) return ret;
    if ((ret = fto_function_fromPolyPiecewise2D(dy, &grad_out->funcs[1])) != FTO_OK) return ret;
    return FTO_OK;
}
...
extern enum FtoError fto_calc_grad(struct FtoGenericFunc *function, struct FtoVectorFunc *grad_out)
{
    ...
    else if (function->type == FTO_POLY_PIECEWISE_2D)
    {
        return fto_calc_grad_piecewise2d(function->state, grad_out);
    }
    else
    {
